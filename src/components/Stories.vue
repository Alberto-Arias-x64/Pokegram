...
<template>
    <section class="cards">
        <div class="cards_container" ref="story">
            <div class="card" v-for="{image, name} in poke_array" :key="image" :src="image" alt="">
            <img class="profile_photo" :src="image" alt="">
            <p>{{name}}</p>
        </div>
        </div>
        <ion-icon id="arrow_left" @click="scroll" name="arrow-back-circle"></ion-icon>
        <ion-icon id="arrow_right" @click="scroll" name="arrow-forward-circle"></ion-icon>
    </section>
</template>
